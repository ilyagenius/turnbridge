...
func pbICE(d []byte) (res []turnCred2) {
	for o := 0; o < len(d); {
		t, n := pbVar(d, o)
		if n == o {
			break
		...
		case 2:
			...
			for _, f := range []uint64{5, 9} {
				for _, blk := range pbAll(inner, f) {
					...
					for _, u := range urls {
						res = append(res, turnCred2{string(u), un, pw})
					}
					for _, blk2 := range pbAll(inner, f) {
						...
						for _, u := range pbAll(blk2, 1) {
							res = append(res, turnCred2{string(u), u2, p2})
						}
					}
					return
				}
...
func dedup(cc []turnCred2) (r []turnCred2) {
